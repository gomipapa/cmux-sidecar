...
- `--auth=none|password` — code-server 인증 모드 (기본 `none`. 127.0.0.1 전용 운영이라 권장)
...
| 이름 | 기본값 | 용도 |
|---|---|---|
| `CMUX_SIDECAR_URL` | `http://127.0.0.1:8765` | code-server URL (포트는 wrapper가 `--bind-addr`로 강제, 8080·8000 등 dev 흔한 포트 회피) |
